...
export class MeetingRoomService {
  ...
  async create(meetingRoomDto: CreateMeetingRoomDto) {
    ...
    if (room) throw new BadRequestException('会议室名字已存在');
  ...
  async update(meetingRoomDto: UpdateMeetingRoomDto) {
    ...
    const meetingRoom: MeetingRoom = await this.repository.findOneBy({
      id: meetingRoomDto.id
    })
    if (!meetingRoom) {

      let {id, ...neoRoom}= meetingRoomDto // 解构出CreateMeetingRoomDto
      // 5 {
      //   name: '天王星',
      //   capacity: 2,
      //   location: '5',
