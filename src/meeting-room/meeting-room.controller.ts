...
export class MeetingRoomController {
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: '会议室名字已存在' })
  @ApiResponse({ status: HttpStatus.OK, type: MeetingRoomVo })
  @Post('create')
  ...
  @Put('update')
  async update(@Body() meetingRoomDto: UpdateMeetingRoomDto): Promise<string> {
    return await this.meetingRoomService.update(meetingRoomDto);
  }

  @ApiBearerAuth()
  @ApiParam({ name: 'id', type: Number })
  @ApiResponse({ status: HttpStatus.OK, description: 'success', type: MeetingRoomVo })
  @Get(':id')
  async find(@Param('id') id: number): Promise<MeetingRoom> {
